...
# Local bin folder
if [ -d "$HOME/felix/local/bin" ] ; then
	PATH="$PATH:$HOME/felix/local/bin"
fi
...
echo "Imitation is the sincerest form of flattery."
#echo "Our task as [humans] is to find the few principles that will 
...
if [[ "$TERM" != dumb ]]; then
    ...
    alias la="command ls -lha ${ls_options:+${ls_options[*]}}"
    #a1# List files with long colored list, without dotfiles (\kbd{ls -l \ldots})
    alias ll="command ls -lh ${ls_options:+${ls_options[*]}}"
